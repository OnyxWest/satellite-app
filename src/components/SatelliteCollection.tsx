"use client";
import React, {useEffect, useRef, useState, FC, MutableRefObject} from "react";
...
const SatelliteCollection: FC<SatelliteCollectionProps> = (props) => {
  ...
  const collectionRef: MutableRefObject<CesiumComponentRef<typeof PointPrimitiveCollection> | null> = useRef(null);
  ...
  useEffect(() => {
    const timer = setInterval(() => {
      ...
      const end = Math.min(start + props.batch_size, satRecsRef.current.length);
      updateSatellitePositions(satRecsRef.current, time, start, end);

      // Update batch index cyclically
      batchIndexRef.current = (batchIndexRef.current + 1) % Math.ceil(satRecsRef.current.length / props.batch_size);
    }, props.update_rate);
